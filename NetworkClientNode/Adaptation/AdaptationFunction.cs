...
namespace NetworkClientNode.Adaptation
{
    public class ClientData : EventArgs 
    {
        ...
        public string ToString()
        {
            ...
            {
                StreamData stream = item.Key;
                string container = stream.VcLevel.ToString();
                container = container.Substring(1, container.Length - 1);
                string stremId = stream.Port + " " + container + " " + stream.Stm + " [" + stream.HigherPath + "," + stream.LowerPath + "]";
                returnString += "At "+ stremId + " : " +  item.Value + "\n";
            }
            return returnString;
        }
    }
    ...
    public class AdaptationFunction
    {
        ...
        public void GetDataFromFrame(object sender, FrameArgs args)
        {
